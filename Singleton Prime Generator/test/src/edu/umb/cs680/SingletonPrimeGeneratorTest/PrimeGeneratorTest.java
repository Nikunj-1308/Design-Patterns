package edu.umb.cs680.SingletonPrimeGenerator;
...
public class PrimeGeneratorTest {
	...
	public void initializeNonNullLinkedListWithoutGeneratingPrimes() {
		...
		Assertions.assertNotNull(cut.getPrimes()); // Check if initialization is done, when instances of class is
													// created
	}
	...
	public void verifyGetPrimesWithoutCallingGeneratePrimesForPrimeBetween1And5() {
		...
		if (cut.primes.size() != 0) {
			cut.primes.clear(); // if primes list member of class was assigned before with some values, clear
								// those
		}
	...
	public void primeBetween1And5() {
		...
		Assertions.assertTrue(cut instanceof SingletonPrimeGenerator); // Check for object to be an instance of class
		Assertions.assertArrayEquals(expectedArray, actualArray); // Check if list of primes in 'expectedArray' is same
																	// as in 'actualArray'
	}
	...
	public void noPrimeBetween14And16() {
		...
		Assertions.assertArrayEquals(expectedArray, actualArray); // Check if list of primes in 'expectedArray' is same
																	// as in 'actualArray'
		Assertions.assertEquals(expectedArray.length, actualArray.length); // Check if size of list of primes
																			// 'expectedArray' is equal to 'actualArray'
	}
	...
	public void primeBetween5And1() {
		try {
			...
			Assertions.assertEquals("Wrong input values: from=5 to=1", ex.getMessage()); // RuntimeException raised as
	...
	public void primeBetweenNeg1And10() {
		try {
			...
			Assertions.assertEquals("Wrong input values: from=-1 to=10", ex.getMessage()); // RuntimeException raised as
																							// 'from' is less than 1
		}
	}
	...
	public void primeBetweenNeg1AndNeg10() {
		try {
			...
			Assertions.assertEquals("Wrong input values: from=-1 to=-10", ex.getMessage()); // RuntimeException raised
																							// as 'from' is less than 1
																							// and 'to' is not greater
																							// than 'from'
		}
	}
	...
	public void primeBetweenNeg13AndNeg13() {
		try {
			...
			Assertions.assertEquals("Wrong input values: from=-13 to=-13", ex.getMessage()); // RuntimeException raised
																								// as 'to' is not
																								// greater than 'from'
		}
	}
	...
	public void verifyGetInstanceCreatesInstanceOfSingletonClass() {
		SingletonPrimeGenerator cut = SingletonPrimeGenerator.getInstance();
		Assertions.assertTrue(cut instanceof SingletonPrimeGenerator); // Check for object to be an instance of class
																		// using getInstance returns a non-null value.
	}
	...
	public void getInstanceReturnsIdenticalInstance() {
		...
		Assertions.assertSame(cut_1, cut_2); // Check getInstance() returns the identical instance when it is called
												// multiple times
	}
	...
	public void primeBetween1And10() {
		...
		Assertions.assertIterableEquals(expected, actual); // Check with assertIterableEquals if getPrimes() returns the
															// expected result
	}
	...
	public void primeBetweenNeg13AndNeg10() {
		...
		try {
			...
			Assertions.assertEquals("Wrong input values: from=-13 to=-10", ex.getMessage()); // RuntimeException raised
																								// as 'from' is less
																								// than 1
		}
	}
	...
	public void primeBetween5And5() {
		try {
			...
			Assertions.assertEquals("Wrong input values: from=5 to=5", ex.getMessage()); // RuntimeException raised as
																							// 'from' is not less than
																							// 'to'
		}
	}
}